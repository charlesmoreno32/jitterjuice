...
def get_bottle_plan():
    ...
    with db.engine.begin() as connection:
        ...
        inventory = connection.execute(sqlalchemy.text("""
                                                      ...
                                                      FROM potion_ledger
                                                      """)).scalar_one()
        potions = connection.execute(sqlalchemy.text("""SELECT potions.id, potions.potion_type, potions.sku, SUM(potion_ledger.potion_change) as inventory
                                                     FROM potions
        ...
        while(inventory < 300 and times < count):
            ...
            for potion in potion_lst:
                ...
                if(inventory < 300 and inv + quants[potion.sku] < 27 and potion.potion_type[0] <= red_ml and potion.potion_type[1] <= green_ml and potion.potion_type[2] <= blue_ml and potion.potion_type[3] <= dark_ml):
